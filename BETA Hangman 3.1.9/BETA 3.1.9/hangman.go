...
func InputUser(hangman1 Hangman) {
	...
	if len(Mo) == 1 { // si la lettre est bien unique
		...
		if IsUpper(Mo) == true {
			...
			fmt.Println(hangman1.usedletters)
			for i := 0; i < len(hangman1.usedletters); i++ {
				if Mo == string(hangman1.usedletters[i]) {

					hangman1.isanusedletter = hangman1.isanusedletter + 1
					fmt.Print(hangman1.isanusedletter)
					fmt.Println(hangman1.isanusedletter, hangman1.usedletters[i], hangman1.usedletters)

				}
				if hangman1.isanusedletter > 1 {
					fmt.Print("déjà utilisé")
					hangman1.usedletters = append(hangman1.usedletters[:i], hangman1.usedletters[i+1:]...)
		...
		var wordverif []string
		for i := range hangman1.userletter { // parcours du mot choisi
			fmt.Println(string(hangman1.testsctword[i]))
			if string(hangman1.testsctword[i]) == string(hangman1.userletter[i]) {
				wordverif = append(wordverif, string(hangman1.testsctword[i])) // si est l'une des lettre entrée par l'utilisateur et stockées dans les diffrentes variable  on le rajoute au tableau contenant la lettre indice et les lettre à trouver cachées par les enderscore
				fmt.Println(wordverif, "ajout")
				hangman1.nbinwordpresent = hangman1.nbinwordpresent + 1

			}
			fmt.Println(wordverif)
			if hangman1.nbinwordpresent == len(hangman1.testsctword)-1 {
				fmt.Println(len(wordverif), hangman1.nbinwordpresent, wordverif)
				fmt.Println("Vous avez gagné !")
				os.Exit(0)
			}
			fmt.Print(wordverif, hangman1.testsctword, hangman1.nbinwordpresent, len(hangman1.testsctword))
		}
		wordverif = nil
